"""PDF quote generation module for Pricer3D.
...
def _make_color_cell(hex_str, styles):
    ...
    # Build swatch + label as a mini table
    ...
    label = Paragraph(hex_str, ParagraphStyle(
        'ColorLabel', parent=styles['table_cell'],
        fontSize=6, textColor=colors.Color(0.4, 0.4, 0.4), alignment=TA_CENTER,
    ))
...
def _try_load_logo(logo_url: str) -> Optional[RLImage]:
    ...
    try:
        ...
        # Logo URLs are like /static/uploads/logos/xxx.png
...
def generate_pdf_quote(
    ...
    Args:
        items: List of quote history items (dicts with filename, material, color,
               quantity, volume_cm3, weight_g, estimated_time_h, cost_cny, etc.)
        brand_name: Company/brand name
        brand_logo_url: URL path to brand logo image
        brand_phone: Contact phone
        brand_contact_email: Contact email
        brand_address: Company address
        brand_note: Default note/terms text
        member_discount_percent: Discount percentage for members (0-100)
    ...
    # Build header table: logo on left, quote info on right
    ...
    # ── Parameters section (from first item's batch settings) ──
    ...
    def _clean_printer(name):
        ...
        result = []
        for p in parts:
            # Model numbers like A1, P1S, K1, X1C, V2, MK4 -> uppercase
    ...
    for idx, item in enumerate(items, 1):
        ...
        # cost_cny is the TOTAL cost for all quantities (already includes qty multiplication)
        total_cost = round(float(item.get('cost_cny') or 0), 2)
        unit_price = round(total_cost / qty, 2) if qty > 0 else total_cost
        subtotal = total_cost  # cost_cny is already the total
    ...
    col_widths = [
        page_width * 0.025,  # #
        page_width * 0.10,   # filename
        28 * mm,             # preview thumbnail
        page_width * 0.06,   # brand
        page_width * 0.10,   # printer
        page_width * 0.06,   # material
        page_width * 0.05,   # color
        page_width * 0.035,  # qty
        page_width * 0.05,   # layer height
        page_width * 0.05,   # infill
        page_width * 0.07,   # weight
        page_width * 0.05,   # time
        page_width * 0.065,  # unit price
        page_width * 0.065,  # subtotal
    ]
    ...
    for i in range(1, len(data_rows)):
        if i % 2 == 0:
            table_style_cmds.append(
                ('BACKGROUND', (0, i), (-1, i), light_gray_color)
            )
    ...
    if member_discount_percent > 0:
        ...
        summary_data.append([
            ...
            Paragraph(f"- ¥ {discount_amount:.2f}", ParagraphStyle(
                'DiscountValue', parent=styles['summary_value'],
        ...
        summary_data.append([
            ...
            Paragraph(f"¥ {final_total:.2f}", ParagraphStyle(
                'FinalTotal', parent=styles['summary_value'],
                fontSize=13,
                textColor=colors.Color(*[c / 255 for c in INDIGO]),
            )),
        ])
    else:
        # Just update the last row to be final total
        summary_data[-1] = [
            Paragraph("应付总额 / Total Due:", styles['summary_label']),
            Paragraph(f"¥ {final_total:.2f}", ParagraphStyle(
                'FinalTotal', parent=styles['summary_value'],
    ...
    # Watermark
    elements.append(HRFlowable(
        width="100%", thickness=0.3, lineCap='round',
