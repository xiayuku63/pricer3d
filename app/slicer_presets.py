...
from .models_orm import SlicerPreset
...
def upsert_slicer_preset(user_id: int, name: str, ext: str, content: bytes) -> dict:
    ...
    with get_db_session() as db:
        ...
        if existing:
            ...
            db.add(row)
            db.flush()
        result = {"id": int(row.id), "name": str(row.name), "ext": str(row.ext), "created_at": str(row.created_at)}
    return result
...
def delete_slicer_preset(user_id: int, preset_id: int) -> bool:
    ...
    if uid <= 0 or pid <= 0:
        return False
    with get_db_session() as db:
        count = db.query(SlicerPreset).filter(SlicerPreset.id == pid, SlicerPreset.user_id == uid).delete()
        return count > 0
