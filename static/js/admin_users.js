...
export function initAdminUsers(root = document) {
    ...
    function setActiveTab(name) {
        ...
        root.querySelectorAll('.tab-btn').forEach((button) => button.classList.toggle('active', button.dataset.tab === name));
        if (!isEmbedded) location.hash = `#${name}`;
    ...
    root.querySelectorAll('.tab-btn').forEach((button) => button.addEventListener('click', () => setActiveTab(button.dataset.tab)));

    const initialTab = (isEmbedded ? '#users' : (location.hash || '#users')).slice(1);
    setActiveTab(['users', 'audit', 'metrics'].includes(initialTab) ? initialTab : 'users');

}

if (document.querySelector('[data-admin-users-page]')) initAdminUsers();
