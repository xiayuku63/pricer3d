...
let _sortState = { key: '', direction: 'asc' };  // key: column key, direction: 'asc'|'desc'
let _paginationState = { page: 1, pageSize: 20 };
...
export function renderResultsTable() {
    ...
    pageItems.forEach((item, pageIdx) => {
        ...
        const statusClass = item.status === 'failed' ? 'table-row-failed' : (item._recalculating ? 'table-row-pending' : 'table-row-success');
        ...
        if (!item._recalculating && item.status === 'success') {
            ...
            toggleBtn.className = 'text-[10px] text-indigo-500 hover:text-indigo-700 underline flex items-center gap-1 mb-1';
            toggleBtn.setAttribute('data-toggle-detail', item.filename);
            toggleBtn.innerHTML = '<svg class="w-3 h-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg> 展开详情（费用明细 / 材料说明 / 打印建议）';

            const detailDiv = document.createElement('div');
            detailDiv.className = 'hidden mt-2 grid grid-cols-1 lg:grid-cols-3 gap-2';
            ...
            card.innerHTML = `
                <div class="flex gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="mt-2 text-[11px] text-red-600">${escapeHtml(item.error || t('common.error'))}</p>
                    </div>
                </div>
